...
#./ch_port_to_6vlan.sh hosts.c29.a output.c29.a summary.c29.a
...
#	if formatted is not None and formatted is not None:
...
def portsec_test(inputlist,session,ipaddress):
	...
	customPrint ("{}:   starting switchport access vlan test".format(ipaddress))
	...
	for interface in inputlist:
		...
		try:
			#show_description = session.send_command('show run | include des')
...
#			test1 = grab_pattern(show_run_int,"(switchport mode access)\n")
#			test2 = grab_pattern(show_run_int,"(switchport access vlan 1002)")
			test1 = grab_pattern(show_run_int,"(switchport mode trunk)")
#			test2 = grab_pattern(show_run_int,"(channel-group)")
#			if test1 is not None and test2 is not None:
			if test1 is not None:
				customPrint("{}:   {} switchport trunk already applied".format(ipaddress,interface))
...
def apply_change(inputlist,session,ipaddress):
	...
	incrementingmac = 1
#	show_description = session.send_command('show run interface {} | include des').format(ipaddress)
#	print(show_description)
...
#	all_the_commands.append ("errdisable recovery interval 30")
	for interface in inputlist:
		all_the_commands.append ("default interface {}".format(interface))
		all_the_commands.append ("interface {}".format(interface))
		all_the_commands.append ("description LSHUT")
		all_the_commands.append ("switchport mode access")	
		all_the_commands.append ("switchport access vlan 6")
		all_the_commands.append ("switchport nonegotiate")
		all_the_commands.append ("switchport port-security maximum 3")
		all_the_commands.append ("switchport port-security violation restrict")
		all_the_commands.append ("switchport port-security aging time 2")
		all_the_commands.append ("switchport port-security aging type inactivity")
		all_the_commands.append ("switchport port-security")
		all_the_commands.append ("ip device tracking maximum 10")
		all_the_commands.append ("load-interval 30")
		all_the_commands.append ("no macro description cisco-desktop")
		all_the_commands.append ("storm-control broadcast level pps 100")
		all_the_commands.append ("storm-control multicast level pps 100")
		all_the_commands.append ("storm-control action trap")
		all_the_commands.append ("spanning-tree portfast")
		all_the_commands.append ("spanning-tree bpduguard enable")
		all_the_commands.append ("switchport port-security")
...
#		incrementingmac = incrementingmac + 1
	...
	customPrint ("")
	customPrint (session.send_command_expect('show archive config differences', read_timeout=50))
	customPrint (session.send_command_expect('write memory', read_timeout=50))
...
print ("{} switches skipped due to uptime less than 90 days".format(skippedswitches_days),file=stats)
...
print ("{} interfaces down less than 90 days".format(skippedinterfaces_days),file=stats)
